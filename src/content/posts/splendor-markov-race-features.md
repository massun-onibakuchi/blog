...
Splendor は先に 15 点以上へ到達した時点で終わるわけではない。同じラウンドで手数を揃えてから勝敗を決める。
...
モデルから見ると同じ入力なのに、本当の状態価値が異なる局面が同じ入力表現に対応付けられることになる。

機械学習的には、状態 `s` を特徴量 `x = φ(s)` へ変換するとき、異なる状態を同じ `x` に写像してしまう representation aliasing になる。
...
つまり、強くなった場合に「新しいネットワーク構造による改善」なのか「欠けていた状態情報による改善」なのか分からなくなるのを避けている。
...
まず未知の局面で WDL Brier が改善するかを見る。同時に policy KL が悪化していないことと、終盤から長い horizon まで value prediction が悪化していないことも確認する。
