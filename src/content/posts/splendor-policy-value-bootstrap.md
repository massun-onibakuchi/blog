...
ここまでの本文は、非公開の実験ログ、コード、計測結果をもとに **ChatGPT が構成して執筆した**。ここからは、実際にこの実装と実験を進めていた自分自身の感想を、あとから振り返って書く。

まず、value が policy より先に過学習したことは、今振り返るとそこまで意外ではなかったと思う。実験中も value だけ validation が怪しくなっている兆候は薄々見えていたし、この時点でもっと早く「これは同じゲームの局面を大量に入れていることが原因では」と気づけたはずだった。

何より、この問題自体は先行研究にかなり直接的に書かれている。

2016年の [AlphaGo 論文](https://deepmind-media.storage.googleapis.com/alphago/AlphaGoNaturePaper.pdf) では、1ゲームの連続する局面は強く相関しているうえ、ゲーム内のすべての局面が同じ最終勝敗を教師信号として持つため、そのまま value network を学習するとゲーム結果を暗記して過学習した、と報告されている。その対策として、value の学習データは **1ゲームから1局面だけ**取り、3,000万局面をそれぞれ別の自己対局から生成していた。

なお、これは AlphaGo Zero ではなく、その前の AlphaGo の論文の話だった。AlphaGo Zero の方は、直近の自己対局から局面をサンプリングして policy と value を同時に学習している。
...
当時、自分は手数や局面の進行度に応じて教師信号の重みを変えるような実装も見ていた記憶がある。ただ、この部分は今の記憶だけで断定したくない。少なくとも、今回のように「ゲーム内の全局面へ最終勝敗を同じ重さで貼る」のが唯一の方法ではなく、先行実装には相関や信用割り当てを弱める工夫がすでにあった。
...
今回を振り返ると、先行研究を読んでいたことで省けた試行錯誤と、読んでいたのに実装へ反映しきれず踏んだ落とし穴の両方があった。特に value の過学習については、学習曲線を見た時点で「既知のあの問題では」と結びつけるべきだった。

次に value の教師データを作り直すときは、単純に局面数を増やすのではなく、**1ゲームから使う局面数を減らす、局面間の相関を考えてサンプリングする、探索由来の値を混ぜる**といった方向を試したい。